...
exports.setViz3_SVG = setViz3_SVG;
exports.setLegendViz3 = setLegendViz3;
exports.setViz3_xScale = setViz3_xScale;
exports.setViz3_yScale = setViz3_yScale;
...
function setCanvasSize(width, height) {
  d3.select('.viz1-container').select('svg').attr('width', width).attr('height', height).attr("padding-top", "25px");
  d3.select('.viz3-container').select('svg').attr('width', width).attr('height', height).attr("padding-top", "25px");
}

function setViz3_SVG(margin_left) {
  d3.select(".viz3-svg").append("g").attr("class", "chart-group").attr("transform", "translate(" + margin_left + ", 0)");
}

function setLegendViz3(width, margin_top) {
  d3.select(".viz3-svg").append("g").attr("class", "legend-viz3").attr("transform", "translate(" + width + ", " + margin_top + ")");
}

function setViz3_xScale(margin, width, height) {
  // X scale
  var xScale = d3.scaleLinear().domain([0, 1]).range([margin.left, width - margin.right / 2]); // X axis

  d3.select(".chart-group").append("g").attr("class", "x axis").call(d3.axisBottom(xScale).ticks(5).tickFormat(d3.format(".0%"))).attr("transform", "translate(-1, " + height + ")");
  return xScale;
}

function setViz3_yScale(margin, height) {
  // Y scale
  var yScale = d3.scaleBand().domain(["media", "non-media"]).range([margin.bottom, height - margin.top]); // Y axis

  d3.select(".chart-group").append("g").attr("class", "y axis").call(d3.axisLeft(yScale)).attr("transform", "translate(" + (margin.left - 1) + "," + margin.top + ")");
  return yScale;
}

function write_text() {
  var viz1_container = d3.select(".viz1-container").insert("div", ":first-child").attr("class", "title-text").append("text").text("TITRE VIZ 1");
  viz1_container.insert("div", ":first-child").attr("class", "text-class").append("text").text("Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Hello.");
  var viz3_container = d3.select(".viz3-container").insert("div", ":first-child").attr("class", "title-text").append("text").text("TITRE VIZ 3");
  viz3_container.insert("div", ":first-child").attr("class", "text-class").append("text").text("Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.");
}
...
function drawLegend(colorScale, g) {
  ...
  var legendOrdinal = (0, _d3SvgLegend.legendColor)().shape("path", d3.symbol().type(d3.symbolCircle).size(150)()).title('Légende').scale(colorScale);
  g.call(legendOrdinal);
  g.select('.legendTitle').attr('font-family', 'Open Sans Condensed');
  g.selectAll('.label').attr('font-size', '12px');
}
},{"d3-svg-legend":"node_modules/d3-svg-legend/indexRollupNext.js"}],"scripts/viz.js":[function(require,module,exports) {
"use strict";
...
exports.drawStackedBar = drawStackedBar;
...
function drawButtons(json_data, color, xScale, yScale) {
  d3.select(".viz3-svg").append("g").attr("class", "buttons-container").attr("transform", "translate(80, 10)");
  d3.select(".buttons-container").selectAll("g").data(Object.keys(CHART_TYPE)).enter().append("g").attr("class", "button").attr("width", 100).attr("height", 30).attr("transform", function (d, i) {
    return "translate(" + i * 110 + ",0)";
  }).on("click", function (d) {
    buttonHandler(d, json_data, color, xScale, yScale);
  }).append("rect").attr("width", 100).attr("height", 30).attr("fill", function (d) {
    return d == 'interactions' ? "#A5A5A5" : "#E0E0E0";
  ...
  d3.select(".buttons-container").selectAll("text").data(Object.keys(CHART_TYPE)).enter().append("text").attr("x", function (d, i) {
    return i * 110;
  }).attr("y", 15).attr("transform", "translate(50,0)").attr("text-anchor", "middle").attr("dominant-baseline", "middle").attr("class", "button-text").text(function (d) {
    return CHART_TYPE[d];
  }).attr("font-size", "10px").attr("fill", "#0000000");
}

function buttonHandler(chart_id, data, color, xScale, yScale) {
  drawStackedBar(data, color, chart_id, xScale, yScale);
  (0, _legend.drawLegend)(color, d3.select('.legend-viz3'));
}

function drawStackedBar(json, color, chart_id, xScale, yScale) {
  var subgroups = Object.keys(json[chart_id]["non-media"]["relative"]);
  var data = Object.keys(json[chart_id]).map(function (page_type) {
    var item = {};
    Object.keys(json[chart_id][page_type]["relative"]).forEach(function (key) {
  ...
  color.domain(subgroups); // Stacked bar

  var stackedData = d3.stack().keys(subgroups).order(d3.stackOrderDescending)(data);
  d3.select(".chart-group").append("g").selectAll("g").data(stackedData).enter().append("g").attr("fill", function (d) {
    return color(d.key);
  ...
  }).attr("height", 60).attr("transform", "translate(0," + 90 + ")").transition().duration(500).attr("width", function (d) {
    return xScale(d[1]) - xScale(d[0]);
  });
}
},{"./legend.js":"scripts/legend.js"}],"scripts/sankey.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.draw_sankey = draw_sankey;

function computeSankey(sankeyWidth, height) {
  var sankey = d3.sankey().nodeId(function (d) {
    return d.name;
  }).nodeAlign(d3.sankeyCenter).nodeWidth(15).nodePadding(10).extent([[1, 5], [sankeyWidth - 1, height - 5]]);
...
function formatData(data) {
  var links = data;
  var nodes = Array.from(new Set(links.flatMap(function (l) {
...
function drawNodes(nodes, color) {
  d3.select(".viz1-svg").append("g").attr("stroke", "#000").selectAll("rect").data(nodes).join("rect").attr("x", function (d) {
    return d.x0;
  }).attr("y", function (d) {
    return d.y0;
...
function addLabels(nodes, width, height) {
  d3.select(".viz1-svg").append("g").attr("font-family", "sans-serif").attr("font-size", 10).selectAll("text").data(nodes).join("text").attr("x", function (d) {
...
function drawLinks(links) {
  var link = d3.select(".viz1-svg").append("g").attr("fill", "none").attr("stroke-opacity", 0.5).selectAll("g").data(links).join("g").style("mix-blend-mode", "multiply");
  link.append("path").attr("d", d3.sankeyLinkHorizontal()).attr("stroke", function (d) {
    return "#aaa";
...
function draw_sankey(color) {
  //(json, color, chart_id, xScale, yScale) {
  var width = 960;
  var height = 625; // const color = d3.scaleOrdinal(d3.schemeCategory10);

  d3.csv('./data_sankey.csv').then(function (data) {
    data = formatData(data); // const svg = d3.select(".viz3-svg").append("svg")
    // .attr("viewBox", [0, 0, width, height]);

    var sankey = computeSankey(width, height);
    var s = sankey(data);
...
exports.height = exports.width = exports.margin = exports.svgSize = exports.colorScheme = void 0;
var colorScheme = ["#8BD98E", "#E0E0E0", "#639FEF", "#AB80DA", "#E98282", "#ECE86B", "#EB992D", "#EC9AF5", "#9af5e0", "#595cf7"];
...
var sankey = _interopRequireWildcard(require("./scripts/sankey.js"));

var legend = _interopRequireWildcard(require("./scripts/legend.js"));

var panel = _interopRequireWildcard(require("./scripts/panel.js"));

var constants = _interopRequireWildcard(require("./scripts/constants.js"));
...
(function (d3) {
  ...
  function build() {
    ...
    sankey.draw_sankey(colorSankey); // VIZ 3
...
},{"./scripts/helper.js":"scripts/helper.js","./scripts/viz.js":"scripts/viz.js","./scripts/sankey.js":"scripts/sankey.js","./scripts/legend.js":"scripts/legend.js","./scripts/panel.js":"scripts/panel.js","./scripts/constants.js":"scripts/constants.js"}],"node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
//# sourceMappingURL=/INF8808_ProjetFinal.e31bb0bc.js.map